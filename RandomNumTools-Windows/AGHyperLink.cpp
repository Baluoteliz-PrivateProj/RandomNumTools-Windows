...
void CAGHyperLink::OnStnClicked()
{
	...
	if (L"" != m_strURL)
		hInstance = ::ShellExecute(GetParent()->GetSafeHwnd(), _T("open"), m_strURL, NULL, NULL, SW_MAXIMIZE);
	if (L"" != m_strCmd)
		hInstance = ::ShellExecute(NULL, _T("open"), L"notepad.exe", m_strCmd, NULL, SW_MAXIMIZE);
...
void CAGHyperLink::SetCmd(CString strCmd)
{
	m_strCmd = strCmd;
}

CString CAGHyperLink::GetCmd() const
{
	return m_strCmd;
}
